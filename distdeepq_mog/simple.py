...
import distdeepq_mog
from distdeepq_mog.replay_buffer import ReplayBuffer, PrioritizedReplayBuffer
...
class ActWrapper(object):
    ...
    def load(path, num_cpu=16):
        ...
        act = distdeepq_mog.build_act(**act_params)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for t in range(max_timesteps):
            ...
            if done and print_freq is not None and len(episode_rewards) % print_freq == 0:
                ...
                logger.record_tabular("Success rate", np.mean(avg_success_list))
                logger.record_tabular("Collision rate", np.mean(avg_collision_list))
                logger.record_tabular("Derailment rate", np.mean(avg_derail_list))
                logger.dump_tabular()
